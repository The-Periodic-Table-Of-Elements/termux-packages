...
termux_step_make_install() {
	local OPENSSL_BINARY=$TERMUX_PREFIX/bin/openssl
	local OPENSSL_INCLUDE=$TERMUX_PREFIX/include/openssl

	[ -f $OPENSSL_BINARY ] && mv $OPENSSL_BINARY{,.tmp}
	[ -d $OPENSSL_INCLUDE ] && mv $OPENSSL_INCLUDE{,.tmp}

	# TODO(@thunder-coding): Also install manpages
	# make -j 1 install ...
	...
	install -m755 -d ${OPENSSL_INCLUDE}3
	mv ${OPENSSL_INCLUDE}{,3}

	[ -f $OPENSSL_BINARY.tmp ] && mv $OPENSSL_BINARY{.tmp,}
	[ -d $OPENSSL_INCLUDE.tmp ] && mv $OPENSSL_INCLUDE{.tmp,}
}
